package com.lingdaoyi.cloud.utils.base;
...
public class IdcardValidator {
	 ...
	 * 敏感字段改为*
     * @param oldStr 原始字符串
     * @param replaceStr 要替换成的字符 如（*）
     * @param start 替换开始位置
     * @param end	替换结束位置
     * @return
     */
    public static String updateStr(String oldStr, String replaceStr, Integer start, Integer end) {
    	
    	String oldChar = oldStr.substring(start, end);
    	int length = oldChar.length();
    	StringBuffer sb = new StringBuffer();
    	for(int i = 0;i<length;i++) {
    		sb = sb.append(replaceStr);
    	}
    	oldStr = oldStr.replace(oldChar, sb);	
    	return oldStr;
    }
}
